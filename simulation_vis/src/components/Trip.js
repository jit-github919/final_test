...
// 시뮬레이션 시간: 0분 = 07:00, 180분 = 10:00
const BASE_MIN = 420;   // 7 * 60
const MAX_TIME = 180;   // 3시간만 돌리기
const mapStyle = "mapbox://styles/spear5306/ckzcz5m8w002814o2coz02sjc";


// mapbox API 키 (개인 키를 입력해서 사용)
const MAPBOX_TOKEN = `pk.eyJ1IjoibW9iaWxlc2luIiwiYSI6ImNtaGJtNWMzbzI0bHUyam9hZHc1cXpycGQifQ.VPE5G4IISHHj3z3Z863kLQ`;
...
/*
  ...
  - time: 시뮬레이션 시간(0~MAX_TIME, 분 단위)
  - BASE_MIN(=420) 을 더해서 07:00~10:00로 표시
*/
const returnAnimationDisplayTime = (time) => {
  const absoluteMinutes = Math.round(time) + BASE_MIN;   // 0 -> 420(07:00)

  const hour = addZeroFill(parseInt((absoluteMinutes / 60) % 24));
  const minute = addZeroFill(absoluteMinutes % 60);
  return [hour, minute];
};
...
const Trip = (props) => {
  ...
  // 시뮬레이션 전체 시간은 0~MAX_TIME(=180분)으로 고정
  ...
  const layers = [
    ...
    new TripsLayer({  
      ...
      // 밝은 시안/하늘색 계열
      getColor: [80, 210, 255],     // RGB (하늘색)
      opacity: 0.95,                // 약간만 투명
      widthMinPixels: 6,            // 선 폭 (조금 두껍게)
      rounded: true,
      capRounded : true,
      jointRounded : true,
      // 꼬리 길이 (단위: timestamp 분) → 4분 정도 꼬리 남게
      trailLength : 4,
      currentTime: time,
      shadowEnabled: false
    }),

    // 2) 승객: 대기 중인 승객 위치 (밝은 하얀 점 + 얇은 하늘색 테두리)
    new ScatterplotLayer({
      id: 'passengers',
      data: visiblePassengers,
      getPosition: d => d.location,
      getFillColor: [255, 255, 255],              // 흰색
      getLineColor: [120, 220, 255],              // 하늘색 아웃라인
    ...
    new ScatterplotLayer({
      ...
      getPosition: d => d.location,
      // 강렬한 오렌지/레드
      getFillColor: [255, 120, 60],
      getLineColor: [255, 200, 160],
    ...
    // 4) 매칭된 차량-승객 링크: 부드러운 하얀 아크 (픽업 전까지)
    new ArcLayer({
      id: 'match-arcs',
      data: (() => {
        if (!trip || trip.length === 0) return [];
      ...
      // 살짝 푸른빛 도는 흰색
      getSourceColor: [220, 230, 255],
      getTargetColor: [220, 230, 255],
      getWidth: 2,
      pickable: false,
      updateTriggers: {
    ...
    // 5) 승객 탑승 중: 현재 차량 위치 → 목적지까지 강렬한 레드/오렌지 아크
    new ArcLayer({
      id: 'occupied-arcs',
      data: (() => {
        if (!trip || trip.length === 0) return [];
      ...
      getTargetPosition: d => d.target,
      // 레드+오렌지 계열 (강렬)
      getSourceColor: [255, 90, 80],
      getTargetColor: [255, 150, 110],
      getWidth: 2,
      pickable: false,
      updateTriggers: {
        getSourcePosition: [time],
